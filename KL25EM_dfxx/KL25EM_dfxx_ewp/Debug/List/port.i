#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\port\\port.c"
...
#line 18 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\port\\port.h"
...
#line 78 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\port\\port.h"


#line 14 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\port\\port.c"
...
{
    ((((SIM_MemMapPtr)0x40047000u))->SCGC5) |= (0x200u << ((ptxn)>>5));       //开启PORTx端口

    ((PORTX[((ptxn)>>5)])->PCR[((ptxn)&0x1f)]) = cfg;         // 复用功能 , 确定触发模式 ,开启上拉或下拉电阻
}
...
void  port_init_NoALT(PTxn ptxn, uint32_t cfg)
{
    ...
    cfg &= ~0x700u;                      //清了MUX 字段（即不需要配置ALT，保持原来的ALT）
    cfg |=  (((PORTX[((ptxn)>>5)])->PCR[((ptxn)&0x1f)]) & 0x700u);    //读取寄存器里配置的 MUX
