#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\i2c\\i2c.c"
...
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\i2c\\i2c.h"
/******************** (C) COPYRIGHT 2011 蓝宙电子工作室 ********************
 * 文件名       ：i2c.h
 * 描述         ：I2C头文件
 * 备注         ：修改自官方例程
...
#line 17 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\i2c\\i2c_cfg.h"
...
#line 17 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\i2c\\i2c.h"
...
#line 15 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\i2c\\i2c.c"


volatile struct I2C_MemMap *I2Cx[2] = {((I2C_MemMapPtr)0x40066000u), ((I2C_MemMapPtr)0x40067000u)}; //定义两个指针数组保存 I2Cx 的地址
...
void iic_init(I2Cn i2cn)
{
  ...
  if(i2cn==I2C0)
  {
    ((((SIM_MemMapPtr)0x40047000u))->SCGC4) |=0x40u;
    /* 配置 I2C0功能的 GPIO 接口 */
    if((PTB2 == PTB0) || (PTB2 == PTB2) ||PTB2 == PTC8 )
      port_init (PTB2, ALT2 | PULLUP );
    else
      ;                              //上诉条件都不满足，直接断言失败了，设置管脚有误？
    
    if ((PTB3 == PTB1) ||  (PTB3 == PTB3) ||PTB3 == PTC9 )
      port_init (PTB3, ALT2 | PULLUP );
    else
      ; 
    
    
    ((((I2C_MemMapPtr)0x40066000u))->C1)=0X00;
    ((((I2C_MemMapPtr)0x40066000u))->S)=0x2u;
  ...
  else
  {
    ...
    ((((SIM_MemMapPtr)0x40047000u))->SCGC4) |=0x80u;
    /* 配置 I2C1功能的 GPIO 接口 */
    if(PTC10 == PTE1)
      port_init (PTC10, ALT6 | PULLUP );
    else if(PTC10 == PTC10)
    ...
    if(PTC11 == PTE0)
      port_init (PTC11, ALT6 | PULLUP );
    else if (PTC11 == PTC11)
      port_init (PTC11, ALT2 | PULLUP );
    else
      ;
    
    ((((I2C_MemMapPtr)0x40067000u))->C1)=0X00;
    ((((I2C_MemMapPtr)0x40067000u))->S)=0x2u;
  ...
  ((I2Cx[i2cn])->F)  = ( 0                        // I2C Frequency Divider register (I2Cx_F)  I2C分频寄存器   I2C最大波特率为 400k
                            | (((uint8_t)(((uint8_t)(mult))<<6))&0xC0u)        // 乘数因子 mul = MULT + 1
                            | (((uint8_t)(((uint8_t)(17))<<0))&0x3Fu)          // 时钟速率 = ICR_2_SCL_divider[ICR] ，查表获得 ICR 与 SCL_divider 映射关系
                            );

  
  /* 使能 IIC1 */
  ((I2Cx[i2cn])->C1) = 0x80u  ;     //使能I2C
...
void I2C_StartTransmission (I2Cn i2cn, uint8_t SlaveID, MSmode Mode)
{
    ...
    /* send start signal */
    ((I2Cx[i2cn])->C1) |= (0x10u ); ((I2Cx[i2cn])->C1) |= ( 0x20u);
...
uint8_t I2C_ReadAddr(I2Cn i2cn, uint8_t SlaveID, uint8_t Addr)
{
    ...
    while(( ((I2Cx[i2cn])->S) & 0x2u)==0) {} ((I2Cx[i2cn])->S) |= 0x2u;;
    ...
    ((I2Cx[i2cn])->C1) &= ~(0x20u ); ((I2Cx[i2cn])->C1) &= ~(0x10u) ;;
...
void I2C_WriteAddr(I2Cn i2cn, uint8_t SlaveID, uint8_t Addr, uint8_t Data)
{
    ...
    (((I2Cx[i2cn])->D) = (Addr));;                    //写地址
    while(( ((I2Cx[i2cn])->S) & 0x2u)==0) {} ((I2Cx[i2cn])->S) |= 0x2u;;

    (((I2Cx[i2cn])->D) = (Data));;                    //写数据
    while(( ((I2Cx[i2cn])->S) & 0x2u)==0) {} ((I2Cx[i2cn])->S) |= 0x2u;;
