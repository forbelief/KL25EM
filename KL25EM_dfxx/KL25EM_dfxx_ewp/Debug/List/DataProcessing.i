#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DataProcessing.c"
/**************************************************************************************************
 * 文件：	DataProcessing.c
...
#line 15 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DataProcessing.h"
...
#line 11 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DataProcessing.c"
 ...
 * 返回：
 *		coords：    一维坐标值
 * 备注：	完全假设是线性关系情况下的模型，对于这个函数不理解的，请qq：1023299899，宋星
 */
int16_t ResultAD2XY(ResultAD_T ResultAD[])
{
	...
	ratio   = 200 * 100 / 100;		//乘100是为了移动小数点，避免浮点计算
	if(LeftV > 0 && RightV > 0)
	{
	...
	{
		coords = (RightV - (100 * 3 / 2)) * (-ratio) / 100;		//(RightV - NormalRangeAD * 1.5) * (-X_LengthMM / NormalRangeAD)
	}
	else if(LeftV > 0 && RightV == 0)
	{
		coords = (LeftV - (100 * 3 / 2)) * ratio / 100;
	}
	else
	{
		//这是一个悲伤的故事
...
int16_t  ResultAD2PoseError(ResultAD_T ResultAD[])
{
	...
	if(LeftX != 0 && RightX == 0)
		PoseError = (int16_t)(atan((float)(LeftY) / LeftX) * 180. / 3.14);
	else if(LeftX != 0 && RightX != 0)
		PoseError = (int16_t)((atan((float)(LeftY) / LeftX) + atan((float)(RightY) / RightX)) * 180. / 3.14 / 2.);
	else if(LeftX == 0 && RightX != 0)
		PoseError = (int16_t)(atan((float)(RightY) / RightX) * 180. / 3.14);
...
int_16 XY2Angle(ResultAD_T ResultAD[])
{
	...
	degree = (int16_t)(atanf(slope) * 180. / 3.14);	//角度，角度制
...
Message_T MessageGenerate(DataEM_T DataEM, OutputData_T OutputData)
{
	...
	if(message.RunF == 1)		//以'a'代表message.RunF == TurnOff,'z'代表message.RunF == TurnOn，以防止出错
		message.str[9] = 'a';
	else if(message.RunF == 0)
		message.str[9] = 'z';
...
uint8_t Place2Run(Flag_T flag)
{
	...
	static uint8_t LogF = 1;
	uint8_t temp;
	
	if(flag.PlaceF == 0 && LogF == 1)	//检测到信号
	{
		LogF = 0;
		++cnt;
	}
	else if(flag.PlaceF == 1 && LogF == 0)	//离开信号区
		LogF = 1;
	else
	{}
	temp = cnt;
...
OutputData_T DataProcessing(DataEM_T DataEM)
{
	...
	if(gpio_get(PTE30) == 0)		//控制单车模式还是双车模式
	{
		DataEM.flag.RunF =  0;
		DataEM.message.RunF = 0;
	}
	...
	if(DataEM.flag.RunF ==  0 && DataEM.message.RunF == 0) //开启控制，保证前后车同时启动
	{
		...
		if(DataEM.flag.PlaceFn == 0)	//检测停止信号，只在后车安装干簧管的话，只有后车停止信号有效,
		{
			...
			if(DataEM.distance < 50 && temp > DataEM.message.speed)  //当距离过小时，并且后车的速度大于前车时，后车应减慢速度。
			{
...
#line 352 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DataProcessing.c"
