#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\DMA\\dma.c"
...
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\DMA\\dma.h"
/******************** (C) COPYRIGHT 2011 蓝宙电子工作室 ********************
* 文件名       ： DMA.h
* 描述         ：工程模版实验
*
...
void  enter_critical(void);
void  exit_critical(void);
#line 14 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\DMA\\dma.c"
...
void DMA_PORTx2BUFF_Init(DMA_CHn CHn, void *SADDR, void *DADDR, PTxn ptxn, DMA_BYTEn byten, uint32_t count)
{
    ...
    //断言，检测传递进来参数是否正确
    ;
#line 61 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\DMA\\dma.c"
    ...
    /* 开启时钟 */
    ((((SIM_MemMapPtr)0x40047000u))->SCGC6) |= 0x2u;                     //打开DMA多路复用器时钟
    ((((SIM_MemMapPtr)0x40047000u))->SCGC7) |= 0x100u;                        //打开DMA模块时钟



                                       
    ((((DMAMUX_MemMapPtr)0x40021000u))->CHCFG[CHn])=0x00;
    ((((DMA_MemMapPtr)0x40008000u))->DMA[CHn]. DSR_BCR)|=0x1000000u;
    ...
    ((((DMA_MemMapPtr)0x40008000u))->DMA[CHn]. DAR) =    (uint32)DADDR;                      // 设置目的地址
    ((((DMA_MemMapPtr)0x40008000u))->DMA[CHn]. DCR) =    (0
                       | (((uint32_t)(((uint32_t)(byten))<<20))&0x300000u)
                       | (((uint32_t)(((uint32_t)(byten))<<17))&0x60000u)
                       //| DMA_DCR_SINC_MASK                  //传输后源地址增加(根据位宽)
                       | 0x80000u                  //传输后目的地址增加(根据位宽)
                       | 0x20000000u                  // 0为不停得传输，直到BCR为0；1为一次请求传输一次
                       //| DMA_DCR_START_MASK               //软件触发传输
                       | 0x40000000u                   //硬件触发传输（与上面START二选一）
                       | 0x80u                 //传输完成后硬件自动清ERQ
                       | 0x800000u
                       );
    ...
    n = (uint8)(((uint32)SADDR - ((uint32)(&((GPIO_MemMapPtr)0x400FF000u)->PDIRByte . Byte0))) & 0x3f) * 8;       //最小的引脚号
    ptx0 += n;
    ...
    ((((DMAMUX_MemMapPtr)0x40021000u))->CHCFG[CHn]) &= ~0x80u;                                    //禁止通道CHn 硬件请求
    //DMA_IRQ_CLEAN(CHn);
    /* 开启中断 */
