#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DebugEM.c"
/*
 *文件：	DebugEM.c
...
#line 11 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DebugEM.h"
...
#line 8 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DebugEM.c"
...
{	
	...
	gpio_Interrupt_init(PTE30, GPI_UP_PF, GPI_DISAB);
}
...
void Initbutten(void)
{
	gpio_Interrupt_init(PTA16, GPI_UP_PF, GPI_INP_FALL);		//输入上拉带无源滤波，下降沿触发
	gpio_Interrupt_init(PTA4, GPI_UP_PF, GPI_INP_FALL);		//输入上拉带无源滤波，下降沿触发        
	gpio_Interrupt_init(PTA5, GPI_UP_PF, GPI_INP_FALL);		//输入上拉带无源滤波，下降沿触发
}
...
void ReceiveDebug_UART_ISR()
{
	static uint8_t StrLog[20 + 1], index;		//接收到的字符串以及下标的记录，加1是因为printf需要结束标志'\0'。
	uint8_t temp;											//用于临时存储接收到的字符
	UART_RceiveFP_T RceiveFP;								//计算浮点值时的结构变量

	temp = uart_getchar (UART0);
	//接收的字符范围限定
	if(((temp >= '0' && temp <= '9') || temp == '+' || temp == '-' || temp == '.') && index < 20)			//记录
		StrLog[index++] = temp;
