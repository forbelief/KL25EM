#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\pit\\pit.c"
...
void  enter_critical(void);
void  exit_critical(void);
#line 13 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\pit\\pit.h"
...
#line 14 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\pit\\pit.c"
...
void pit_init(PITn pitn, uint32_t cnt)
{
    ...
    ((((SIM_MemMapPtr)0x40047000u))->SCGC6)       |= 0x800000u;          //使能PIT时钟

    ((((PIT_MemMapPtr)0x40037000u))->MCR)         = (0
                       //| PIT_MCR_MDIS_MASK       //禁用PIT定时器时钟选择（0表示使能PIT，1表示禁止PIT）
                       //| PIT_MCR_FRZ_MASK        //调试模式下停止运行（0表示继续运行，1表示停止运行）
                      );

    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. LDVAL)  = cnt - 1 ;                    //设置溢出中断时间

    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TFLG)|=0x1u;                           //清中断标志位

    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TCTRL) &= ~ 0x1u;        //禁止PITn定时器（用于清空计数值）
    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TCTRL)  = ( 0
                         | 0x1u        //使能 PITn定时器
                         | 0x2u        //开PITn中断
                       );
...
void pit_delay(PITn pitn, uint32_t cnt)
{
    ...
    ((((PIT_MemMapPtr)0x40037000u))->MCR)         &= ~(0x2u  );    //使能PIT定时器时钟 ，调试模式下继续运行

    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TCTRL) &= ~( 0x1u );                     //禁用PIT ，以便设置加载值生效
    ...
    while( !(((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TFLG)& 0x1u));

    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TFLG)|=0x1u;                                           //清中断标志位
    
    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TCTRL) &= ~ 0x1u;                        //禁止（加）
}
...
void pit_time_start(PITn pitn)
{
    ...
    ((((PIT_MemMapPtr)0x40037000u))->MCR)         &= ~(0x2u | 0x1u );    //使能PIT定时器时钟 ，调试模式下继续运行

    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TCTRL) &= ~( 0x1u );                     //禁用PIT ，以便设置加载值生效

    ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. LDVAL)  = ~0;                                          //设置溢出中断时间
...
uint32_t pit_time_get(PITn pitn)
{
    ...
    val = (~0) - ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. CVAL);

    if(((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TFLG)& 0x1u)                           //判断是否时间超时
    {
        ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TFLG)|=0x1u;                                       //清中断标志位
        ((((PIT_MemMapPtr)0x40037000u))->CHANNEL[pitn]. TCTRL) &= ~ 0x1u;                    //禁止PITn定时器（用于清空计数值）
        return ~0;
