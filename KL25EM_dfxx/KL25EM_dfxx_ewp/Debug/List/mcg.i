#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\mcg.c"
...
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\mcg.h"
/******************** (C) COPYRIGHT 2011  ********************
 * 文件名       ：mcg.h
 * 描述         ：锁相环头文件
 *
...
#line 17 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\mcg.h"
...
/*********************   自定义 时钟频率 分频因子   ********************/
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\Fire_kinetis_MCG_CFG.h"
/******************** (C) COPYRIGHT 2011  ********************
 ...
 * 技术支持论坛 ：
**********************************************************************************/
#line 516 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\Fire_kinetis_MCG_CFG.h"
...
void  enter_critical(void);
void  exit_critical(void);
#line 15 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\mcg.c"
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\mcg_cfg.h"

#line 121 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\mcg_cfg.h"

#line 17 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\mcg.c"
...
//时钟分频因子
struct mcg_div mcg_div = { 11 , 31 , 0 , 0 , 0 , 9 }; //定义的同时，初始化
...
*************************************************************************/
#line 202 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\mcg\\mcg.c"
unsigned char pll_init(clk_option opt)
{
    ...
    if(opt  != PLLUSR ) //自定义模式，直接加载全局变量mcg_div的值
    {
       ...
       case PLL80:
        ...
        if     (opt <= 1 * 30)   mcg_div.flash_div = 0; // flash  = MCG
        else if(opt <= 2 * 30)   mcg_div.flash_div = 1; // flash  = MCG/2
        else if(opt <= 3 * 30)   mcg_div.flash_div = 2; // flash  = MCG/3
        else if(opt <= 4 * 30)   mcg_div.flash_div = 3; // flash  = MCG/4
        else if(opt <= 5 * 30)   mcg_div.flash_div = 4; // flash  = MCG/5
        else if(opt <= 6 * 30)   mcg_div.flash_div = 5; // flash  = MCG/6
        else if(opt <= 7 * 30)   mcg_div.flash_div = 6; // flash  = MCG/7
        else                                mcg_div.flash_div = 7; // flash  = MCG/8

    }
    pll_freq = (unsigned char)(( (unsigned short)50 * (unsigned short)( mcg_div.vdiv + 24 )) / (unsigned short)( mcg_div.prdiv + 1 ) );            //  50/ ( prdiv +1 ) * ( mcg_div.vdiv + 24 )
    ...
    ((((MCG_MemMapPtr)0x40064000u))->C2) = (((uint8_t)(((uint8_t)(2))<<4))&0x30u) | 0x8u | 0x4u;
 ...
 //   LLWU_CS |= LLWU_CS_ACKISO_MASK;
    ...
    ((((MCG_MemMapPtr)0x40064000u))->C1) = (((uint8_t)(((uint8_t)(2))<<6))&0xC0u) | (((uint8_t)(((uint8_t)(7))<<3))&0x38u);

    /* if we aren't using an osc input we don't need to wait for the osc to init */
//#if (!defined(Kl25_CLK) && !defined(ASB817))
    while (!(((((MCG_MemMapPtr)0x40064000u))->S) & 0x2u)) {};  //等待晶振稳定
//#endif

    while (((((MCG_MemMapPtr)0x40064000u))->S) & 0x10u) {}; // wait for Reference clock Status bit to clear

    while (((((((MCG_MemMapPtr)0x40064000u))->S) & 0xCu) >> 2) != 0x2) {}; // Wait for clock status bits to show clock source is ext ref clk

    //进入FBE模式  分频后结果必须在 ：2 MHz ~ 4 MHz.
    //    n       (n+1)分频   50M/(n+1)       n为12~24之间
    ((((MCG_MemMapPtr)0x40064000u))->C5) = (((uint8_t)(((uint8_t)(mcg_div . prdiv))<<0))&0x1Fu);   // prdiv +1 分频 ：2M

    ((((MCG_MemMapPtr)0x40064000u))->C6) = 0x0;       // Ensure MCG_C6 is at the reset default of 0. LOLIE disabled, PLL disabled, clk monitor disabled, PLL VCO divider is clear
    ...
    ((((SIM_MemMapPtr)0x40047000u))->CLKDIV1) = (((uint32_t)(((uint32_t)(mcg_div . core_div))<<28))&0xF0000000u) | (((uint32_t)(((uint32_t)(mcg_div . flash_div))<<16))&0x70000u);
    
    // Set the VCO divider and enable the PLL for 48MHz, LOLIE=0, PLLS=1, CME=0, VDIV=0
    //   n    (n+24)倍频            n为0~31之间
    ((((MCG_MemMapPtr)0x40064000u))->C6) = 0x40u | (((uint8_t)(((uint8_t)(mcg_div . vdiv))<<0))&0x1Fu) ;  //  mcg_div.vdiv + 1 倍频

    while (!(((((MCG_MemMapPtr)0x40064000u))->S) & 0x20u)) {}; // wait for PLL status bit to set

    while (!(((((MCG_MemMapPtr)0x40064000u))->S) & 0x40u)) {}; // Wait for LOCK bit to set

    // 现在已经进入了 PBE 模式

    // Transition into PEE by setting CLKS to 0
    // CLKS=0, FRDIV=3, IREFS=0, IRCLKEN=0, IREFSTEN=0
    ((((MCG_MemMapPtr)0x40064000u))->C1) &= ~0xC0u;

    // Wait for clock status bits to update
    while (((((((MCG_MemMapPtr)0x40064000u))->S) & 0xCu) >> 2) != 0x3) {};
