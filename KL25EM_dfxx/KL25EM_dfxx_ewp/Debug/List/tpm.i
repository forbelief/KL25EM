#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\tpm\\tpm.c"
...
**********************************************************************************/
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\tpm\\tpm.h"
//===========================================================================
//文件名称：tpm.h
//功能概要：tpm底层驱动构件头文件
...
#line 11 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\tpm\\tpm.h"
...
#line 14 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\tpm\\tpm.c"

/*
 * 定义数组
 */

TPM_MemMapPtr TPMN[3] = {((TPM_MemMapPtr)0x40038000u), ((TPM_MemMapPtr)0x40039000u), ((TPM_MemMapPtr)0x4003A000u)}; //定义三个指针数组保存 TPMn_e 的地址
...
void tpm_port_mux(TPMn_e tpmn, TPM_CHn_e ch)
{
    ...
    {
    case TPM0:
        ((((SIM_MemMapPtr)0x40047000u))->SCGC6) |= 0x1000000u;       //使能TPM0时钟
        switch(ch)
        {
        case TPM_CH0:
            if((PTE24 == PTC1) || (PTE24 == PTD0) )
            {
                port_init(PTE24, ALT4);
            }
            else if((PTE24 == PTE24) || (PTE24 == PTA3) )
            {
                port_init(PTE24, ALT3);
            }
            else
            {
                ;                      //设置管脚有误？
        ...
        case TPM_CH1:
            if((PTA4 == PTC2) || (PTA4 == PTD1))
            {
                port_init(PTA4, ALT4);
            }
            else if((PTA4 == PTA4) || (PTA4 == PTE25) )
            {
                port_init(PTA4, ALT3);
            }
            else
        ...
        case TPM_CH2:
            if((PTA5 == PTC3) || (PTA5 == PTD2) )
            {
                port_init(PTA5, ALT4);
            }
            else if((PTA5 == PTA5) || (PTA5 == PTE29) )
            {
                port_init(PTA5, ALT3);
            }
            else
        ...
        case TPM_CH3:
            if((PTC4 == PTC4) || (PTC4 == PTD3) )
            {
                port_init(PTC4, ALT4);
            }
            else if((PTC4 == PTA6) || (PTC4 == PTE30))
            {
                port_init(PTC4, ALT3);
            }
            else
        ...
        case TPM_CH4:
            ...
            else if((PTC8 == PTC8) || (PTC8 == PTA7) || (PTC8 == PTE31) )
            {
                port_init(PTC8, ALT3);
            }
            else
        ...
        case TPM_CH5:
            ...
            else if((PTC9 == PTA0)  || (PTC9 == PTC9)  ||   (PTC9 == PTE26)    )
            {
                port_init(PTC9, ALT3);
            }
            else
    ...
    case TPM1:
        ((((SIM_MemMapPtr)0x40047000u))->SCGC6) |= 0x2000000u;       //使能TPM1时钟
        switch(ch)
        {
        case TPM_CH0:
            if((PTA12 == PTE20) || (PTA12 == PTA12) || (PTA12 == PTB0) )
            {
                port_init(PTA12, ALT3);
            }
            else
            {
                ;                      //设置管脚有误？
        ...
        case TPM_CH1:
            if((PTA13 == PTE21) || (PTA13 == PTA13) || (PTA13 == PTB1) )
            {
                port_init(PTA13, ALT3);
            }
            else
            {
                ;                      //设置管脚有误？
    ...
    case TPM2:
        ((((SIM_MemMapPtr)0x40047000u))->SCGC6) |= 0x4000000u;                           //使能TPM2时钟
        switch(ch)
        {
        case TPM_CH0:
            if((PTB18 == PTA1) || (PTB18 == PTB2) || (PTB18 == PTB18) || (PTB18 == PTE22) )
            {
                port_init(PTB18, ALT3);
            }
            else
            {
                ;                      //设置管脚有误？
        ...
        case TPM_CH1:
            if((PTA2 == PTA2) || (PTA2 == PTB3)|| (PTA2 == PTB19)|| (PTA2 == PTE23))
            {
                port_init(PTA2, ALT3);
            }
            else
...
void TPM_PWM_init(TPMn_e tpmn, TPM_CHn_e ch, uint32_t freq, uint32_t duty)
{
   uint32 clk_hz ;
    ...
    ;         // 断言， PS 最大为 0x07 ，超过此值，则 PWM频率设置过低，或 Bus 频率过高
    ...
    ((TPMN[tpmn])->CONTROLS[ch]. CnSC) &= ~0x4u;
    ((TPMN[tpmn])->CONTROLS[ch]. CnSC)  = 0x20u | 0x8u;
  ...
  //  SIM_SOPT2 &= ~ SIM_SOPT2_TPMSRC_MASK;
    ...
    ((TPMN[tpmn])->SC)    = ( 0
                                  ...
                                  | (((uint32_t)(((uint32_t)(ps))<<0))&0x7u)             //分频因子，分频系数 = 2^PS
                                  | (((uint32_t)(((uint32_t)(1))<<3))&0x18u)						//使能计数器
                                );
    ((TPMN[tpmn])->MOD)   = mod;                        //模数, EPWM的周期为 ：MOD - CNTIN + 0x0001
    ((TPMN[tpmn])->CONTROLS[ch]. CnV) = cv;
    ((TPMN[tpmn])->CNT)   = 0;                          //计数器。只有低16位可用（写任何值到此寄存器，都会清0）
}
...
void TPM_Stop(uint8_t tpmModule)
{
    ...
    {
		case 0:
		  ((((TPM_MemMapPtr)0x40038000u))->SC) &= ~0x18u;
		  break; 
		case 1:
		  ((((TPM_MemMapPtr)0x40039000u))->SC) &= ~0x18u;
		  break; 
		case 2:
		  ((((TPM_MemMapPtr)0x4003A000u))->SC) &= ~0x18u;
		  break; 
    }
}
...
void TPM_PWM_freq(TPMn_e tpmn, uint32_t freq)             //设置TPM的频率
{
    ...
    ((TPMN[tpmn])->MOD)   = mod;                        //模数, EPWM的周期为 ：MOD - CNTIN + 0x0001
    ((TPMN[tpmn])->CNT)   = 0;                          //计数器。只有低16位可用（写任何值到此寄存器，都会加载 CNTIN 的值）
}
...
void TPM_pulse_init(TPMn_e tpmn,TPM_CLKIN_e clkin,TPM_PS_e ps)
{
    ...
    if(clkin == TPM_CLKIN0)
    {
        ((((SIM_MemMapPtr)0x40047000u))->SOPT4) &= ~( 0x1000000u << tpmn );
        port_init(PTC12,ALT4);
    }
    else if(clkin == TPM_CLKIN1)
    {
        ((((SIM_MemMapPtr)0x40047000u))->SOPT4) |= ( 0x1000000u << tpmn );
        port_init(PTC13,ALT4);
    }
    else
    {
        ;              //断言，clkin 只能 0 和 1
    ...
    ((TPMN[tpmn])->SC)    = ( 0
                                  //| TPM_SC_CPWMS_MASK         //0：上升沿计数模式 ，1： 跳变沿计数模式选择 （注释了表示 0）
                                  | (((uint32_t)(((uint32_t)(ps))<<0))&0x7u)             //分频因子，分频系数 = 2^PS
                                  | (((uint32_t)(((uint32_t)(2))<<3))&0x18u)
                               );
    ((TPMN[tpmn])->MOD)   = 0xFFFF;
    ((TPMN[tpmn])->CNT)   = 0;                          //计数器。只有低16位可用（写任何值到此寄存器，都会清0）
}
...
uint16_t     TPM_pulse_get (TPMn_e tpmn)
{
    return (uint16)((TPMN[tpmn])->CNT);                          //计数器。只有低16位可用（写任何值到此寄存器，都会清0）
}
