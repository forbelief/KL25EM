#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\OutputControl.c"
/*
 *文件：	OutputControl.c
...
#line 16 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\OutputControl.h"
...
#line 14 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\OutputControl.c"
...
void InitSpeedPID(void)
{
	...
	cnt = bus_clk_khz * 10 ;
	pit_init(PIT0, cnt);//定时模块初始化
	lptmr_counter_init(LPT0_ALT2, (uint16_t)7, LPT_Rising);//计数模块初始化
...
float PID_DigitalController(float ExpectPID, float BackPID, ParamPID_T ParamPID)
{
	...
	ParamPID.Ti_Ki = ParamPID.Kp_Kp * 10  / ParamPID.Ti_Ki;//单位要换算过来
	ParamPID.Td_Kd = ParamPID.Kp_Kp * ParamPID.Td_Kd / 10 ;
	...
	if(OutPID > (100 * 95 / 100))				//防止积分饱和
		OutPID = 100;
	if(OutPID < (-100 * 95 / 100))
		OutPID = -100;
	return OutPID;
}
...
float ReadSpeedBack(void)
{
	...
	SpeedBack = CountPulse / (500.0 * 105.0 / 40.0) * 5.5 * 3.14 / 10 * 1000; //返回速度，单位cm/s。
...
void ClosePID(void)
{
	pit_time_close(PIT0);			//关闭pit定时器
	
	((((LPTMR_MemMapPtr)0x40040000u))->CSR)  &= ~0x1u;		//禁用LPT的时候就会自动清计数器的值
}
...
void OpenPID(void)
{
	...
	((((PIT_MemMapPtr)0x40037000u))->CHANNEL[PIT0]. TCTRL) &= ~ 0x1u;	//禁止PITn定时器（用于清空计数值）
	((((PIT_MemMapPtr)0x40037000u))->CHANNEL[PIT0]. TCTRL) |= 0x1u;		//使能 PITn定时器
}
...
void MessageSend(Message_T message)
{
	uart_putchar (UART1, 's');			//发送起始字符
	uart_sendN (UART1, message.str, 13);	//发送信息字符
}
 ...
 void PID_PIT_ISR(void)
{
	...
	if((((((PIT_MemMapPtr)0x40037000u))->CHANNEL[PIT0]. TFLG) &= 0x1u) != 0)	//pid
	{
		((((PIT_MemMapPtr)0x40037000u))->CHANNEL[PIT0]. TFLG)|=0x1u;
