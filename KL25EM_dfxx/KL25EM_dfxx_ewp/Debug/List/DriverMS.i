#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DriverMS.c"
/*
 *文件：DriverMS.c
 *目的：电机和舵机的标准化接口的实现。
 *
...
#line 12 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DriverMS.h"
...
#line 13 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DriverMS.h"
...
#line 9 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DriverMS.c"
...
void InitMotor()
{
	...
	switch(TPM1)                //选择占空比的分母
	{
	case TPM0:
		MotorPositivePrecison = 1000u;
		break;
	case TPM1:
		MotorPositivePrecison = 1000u;
		break;
...
#line 62 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DriverMS.c"
	DutyCyclePositive = (0 + 100) * MotorPositivePrecison / (100 + 100);	//计算power所对应的占空比
	TPM_PWM_init(TPM1, TPM_CH0, 10000, DutyCyclePositive);	//初始化TPM 的PWM 功能，速度0	
...
void InitServo()
{
	...
	DutyCycle = (uint16_t)(((500. + (2500.- 500.) * (float)(180 / 2) / 180) / 1000. / 1000.) * (float)300 * (float)ServoPrecison);
	TPM_PWM_init(TPM0, TPM_CH0, 300, DutyCycle);   //初始化TPM 的PWM 功能，90度正中
...
void PowerMotor(int16_t power)
{
	...
	if(power != PowerLog)
	{
#line 139 "F:\\KL25EM\\KL25EM_dfxx\\src\\app\\DriverMS.c"
		DutyCyclePositive = (-power + 100) * MotorPositivePrecison / (100 + 100);	//计算power所对应的占空比
		TPM_PWM_Duty(TPM1, TPM_CH0, DutyCyclePositive);			//设置TPM 的PWM 通道占空比
...
void AngleServo(int16_t angle)
{
	...
	//角度参数限制
	if(angle > (180 * 150 / 180))
		angle = (180 * 150 / 180);
	else if(angle < (180 * 30 / 180))
		angle = (180 * 30 / 180);
...
//后车角度修正
		...
		DutyCycle = (uint16_t)(((500. + (2500.- 500.) * (float)(angle) / 180) / 1000. / 1000.) * (float)300 * (float)ServoPrecison);	//计算angle所对应的占空比
		TPM_PWM_Duty(TPM0, TPM_CH0, DutyCycle);		 //设置TPM 的PWM 通道占空比  
	}
}
