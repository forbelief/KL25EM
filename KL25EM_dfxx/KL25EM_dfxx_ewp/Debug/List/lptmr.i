#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\lptmr\\lptmr.c"
...
**********************************************************************************/
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\lptmr\\lptmr.h"
/******************** (C) COPYRIGHT 2011 蓝宙电子工作室 ********************
* 文件名       ： lptmr.h
* 描述         ：工程模版实验
*
...
#line 18 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\lptmr\\lptmr.h"
...
#line 14 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\lptmr\\lptmr.c"
...
void lptmr_internal_ref_init()
{
	...
	((((MCG_MemMapPtr)0x40064000u))->C1)|=0x2u;      //使能内部参考时钟
	((((MCG_MemMapPtr)0x40064000u))->C2)|=0x1u;		  //MCG_C[IRCS]=1,使能快速内部参考时钟（4MHz）
	((((MCG_MemMapPtr)0x40064000u))->SC) |= (((uint8_t)(((uint8_t)(0x00))<<1))&0xEu) ;
	((((MCG_MemMapPtr)0x40064000u))->C1)|=0x1u;     //停止模式的时候内部时钟保持使能
	((((SIM_MemMapPtr)0x40047000u))->SCGC5)|=0x1u;  //使能LPT模块时钟

	((((LPTMR_MemMapPtr)0x40040000u))->PSR)=(((uint32_t)(((uint32_t)(0x00))<<0))&0x3u)|(((uint32_t)(((uint32_t)(0x09))<<3))&0x78u);//使用内部时钟，1024预分频
	((((LPTMR_MemMapPtr)0x40040000u))->CMR)=(((uint32_t)(((uint32_t)(compare_value))<<0))&0xFFFFu);            //设置比较寄存器值
	((((LPTMR_MemMapPtr)0x40040000u))->CSR)&=(~0x1u);
	((((LPTMR_MemMapPtr)0x40040000u))->CSR)&=(~0x4u);
	((((LPTMR_MemMapPtr)0x40040000u))->CSR)&=(~0x2u);
	((((LPTMR_MemMapPtr)0x40040000u))->CSR)|=0x1u;   //开启LPT模块设置
	((((LPTMR_MemMapPtr)0x40040000u))->CSR)|=0x40u; //开启LPT定时器中断
}
...
void lptmr_counter_init(LPT0_ALTn altn, uint16_t count, LPT_CFG cfg)
{
    ...
    // 清状态寄存器
    ((((LPTMR_MemMapPtr)0x40040000u))->CSR) = 0x00 ;                                          //先关了LPT，这样才能设置时钟分频,清空计数值等
    ((((LPTMR_MemMapPtr)0x40040000u))->PSR) = 0x00 ;
    ((((LPTMR_MemMapPtr)0x40040000u))->CSR) = 0x00 ;
    ((((LPTMR_MemMapPtr)0x40040000u))->CMR) = 0X00 ; 
   
    // 设置累加计数值
    ((((LPTMR_MemMapPtr)0x40040000u))->CMR)  =   (((uint32_t)(((uint32_t)(count))<<0))&0xFFFFu);                   //设置比较值

    ((((LPTMR_MemMapPtr)0x40040000u))->PSR)  =   (0
                     |(((uint32_t)(((uint32_t)(0x00))<<0))&0x3u)
                     |0x4u
                     |(((uint32_t)(((uint32_t)(0x01))<<3))&0x78u)); 
    // 管脚设置、使能中断
    
    ((((LPTMR_MemMapPtr)0x40040000u))->CSR)  =  (0
                    | (((uint32_t)(((uint32_t)(altn))<<4))&0x30u)       // 选择输入管脚 选择
                    | 0x2u        // 选择脉冲计数 (注释了表示时间计数模式)
                    | ( cfg == LPT_Falling ?  0x8u :   0  )  //脉冲计数器触发方式选择：0为高电平有效，上升沿加1
                    //| LPTMR_CSR_TEN_MASK        //使能LPT(注释了表示禁用)
                    //| LPTMR_CSR_TIE_MASK        //中断使能
                    | 0x4u      //0:计数值等于比较值就复位；1：溢出复位（注释表示0）
                   );
...
{
    uint16_t  LPTCOUNT = 0  ;
    ((((LPTMR_MemMapPtr)0x40040000u))->CNR) = 12 ;
    LPTCOUNT = ((((LPTMR_MemMapPtr)0x40040000u))->CNR) ;
    lptmr_pulse_clean();
