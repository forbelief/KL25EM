#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\spi\\SPI.c"
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\spi\\SPI.h"
//==============================================================================
//文件名：SPI.h
//说明：SPI构件头文件。
//==============================================================================
...
void  enter_critical(void);
void  exit_critical(void);
#line 9 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\spi\\SPI.h"
...
#line 2 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\spi\\SPI.c"
...
uint_8 SPI_init(uint_8 SPI_No,uint_8 SPI_mode)
{
	SPI_MemMapPtr baseadd=(SPI_MemMapPtr)(0x40076000u+SPI_No*0x00001000u);
		if(SPI_No==0)		//对把portA的14~17引脚选择为SPI0功能。
		{
			((((SIM_MemMapPtr)0x40047000u))->SCGC4)|=0x400000u;			//打开SPI0模块时钟。
			((((PORT_MemMapPtr)0x40049000u))->PCR[14])=(0|(((uint32_t)(((uint32_t)(0x02))<<8))&0x700u));		//选择PTA14的SS功能
			((((PORT_MemMapPtr)0x40049000u))->PCR[15])=(0|(((uint32_t)(((uint32_t)(0x02))<<8))&0x700u));		//选择PTA15的SCK功能
			((((PORT_MemMapPtr)0x40049000u))->PCR[16])=(0|(((uint32_t)(((uint32_t)(0x02))<<8))&0x700u));		//选择PTA16的MOSI功能
			((((PORT_MemMapPtr)0x40049000u))->PCR[17])=(0|(((uint32_t)(((uint32_t)(0x02))<<8))&0x700u));		//选择PTA17的MIOS功能
			
			((((SPI_MemMapPtr)0x40076000u))->C1) = 0|0x40u
			    		|0x10u
			    		|0x2u;		//对SPI0的C1寄存器配置为主机模式、从机选择自动模式并
			    									//使能SPI0模块。
			((((SPI_MemMapPtr)0x40076000u))->C2) = 0|0x10u;		//与SPI0中C1的SSOE配合，把从机选择配置为自动模式。
			((((SPI_MemMapPtr)0x40076000u))->BR) = 0x00U;					//设置波特率。
		}
		else if(SPI_No==1)		//对把portE的1~4引脚选择为SPI1功能。
		{
			((((SIM_MemMapPtr)0x40047000u))->SCGC4)|=0x800000u;			//打开SPI1模块时钟。
			((((PORT_MemMapPtr)0x4004D000u))->PCR[1])=(0|(((uint32_t)(((uint32_t)(0x02))<<8))&0x700u));		//选择PTE1的MOSI功能
			((((PORT_MemMapPtr)0x4004D000u))->PCR[2])=(0|(((uint32_t)(((uint32_t)(0x02))<<8))&0x700u));		//选择PTE2的SCK功能
			((((PORT_MemMapPtr)0x4004D000u))->PCR[3])=(0|(((uint32_t)(((uint32_t)(0x02))<<8))&0x700u));		//选择PTE3的MISO功能
			((((PORT_MemMapPtr)0x4004D000u))->PCR[4])=(0|(((uint32_t)(((uint32_t)(0x02))<<8))&0x700u));		//选择PTE4的MOSI功能
			
			 ((((SPI_MemMapPtr)0x40077000u))->C1) = 0|0x40u
			    		|0x80u;	   //对SPI1的C1寄存器进行配置
			 enable_irq (11);				       //开启SPI1中断。
			 ((((SPI_MemMapPtr)0x40077000u))->C2) = 0x00U;		          
			 ((((SPI_MemMapPtr)0x40077000u))->BR) = 0x00U;		 		      //设置与SPI0相同的波特率。
...
{
	SPI_MemMapPtr baseadd=(SPI_MemMapPtr)(0x40076000u+SPI_No*0x00001000u);
	while(!(((baseadd)->S)& 0x80u));	//检测SPI1是否收到了数据。
	return ((baseadd)->D);				
}
