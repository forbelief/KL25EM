#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\adc\\adc.c"
...
**********************************************************************************/
#line 1 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\adc\\adc.h"
/******************** (C) COPYRIGHT 2011 蓝宙电子工作室 ********************
* 文件名       ： adc.H
* 描述         ：工程模版实验
*
...
#line 18 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\adc\\adc.h"
...
#line 14 "F:\\KL25EM\\KL25EM_dfxx\\src\\drivers\\adc\\adc.c"

ADC_MemMapPtr ADCN[1] = {((ADC_MemMapPtr)0x4003B000u)}; //定义一个指针数组保存 ADCN 的地址
...
void adc_init(ADCn_Ch_e adcn_ch)
{
  ...
  {
  case ADC0:       /*   ADC0  */
    ((((SIM_MemMapPtr)0x40047000u))->SCGC6) |= (0x8000000u );        //开启ADC0时钟
    ((((SIM_MemMapPtr)0x40047000u))->SOPT7) &= ~(0x80u  | 0x10u);
    ((((SIM_MemMapPtr)0x40047000u))->SOPT7) |= (((uint32_t)(((uint32_t)(0))<<0))&0xFu);
    break;
  ...
  default:
    ;      //断言，传递的管脚不支持 ADC 单端软件触发，请换 其他管脚
    break;
  }
...
uint16_t adc_once(ADCn_Ch_e adcn_ch, ADC_nbit bit) //采集某路模拟量的AD值
{
  ...
  while (( ((ADCN[adcn])->SC1[0]) & 0x80u ) != 0x80u);   //只支持 A通道
  result = ((ADCN[adcn])->R[0]);
  ((ADCN[adcn])->SC1[0]) &= ~0x80u;
  return result;
}
/*************************************************************************
*                             蓝宙电子工作室
...
uint16_t adc_sum(ADCn_Ch_e adcn_ch, ADC_nbit bit, unsigned char N) //采样集合
{
    ...
    for(i = 0; i < N; i++)
    {
      while (( ((ADCN[adcn])->SC1[0]) & 0x80u ) != 0x80u);
      tmp += ((ADCN[adcn])->R[0]);
      ((ADCN[adcn])->SC1[0]) &= ~0x80u;
    }
...
void adc_start(ADCn_Ch_e adcn_ch, ADC_nbit bit)
{
  ...
  ((ADCN[adcn])->CFG1) = (0
                              ...
                              | (((uint32_t)(((uint32_t)(2))<<5))&0x60u)              //时钟分频选择,分频系数为 2^n,2bit
                              | 0x10u          //采样时间配置，0为短采样时间，1 为长采样时间
                              | (((uint32_t)(((uint32_t)(bit))<<2))&0xCu)
                              | (((uint32_t)(((uint32_t)(0))<<0))&0x3u)            //0为总线时钟,1为总线时钟/2,2为交替时钟（ALTCLK），3为 异步时钟（ADACK）。
                                      );
  ...
  ((ADCN[adcn])->CFG2)  = (0
                               ...
                               | 0x4u          //高速配置,0为正常转换序列，1为高速转换序列
                               | (((uint32_t)(((uint32_t)(0))<<0))&0x3u)           //长采样时间选择，ADCK为4+n个额外循环，额外循环，0为20，1为12，2为6，3为2
                                   );
  
  //写入 SC1A 启动转换
  ((ADCN[adcn])->SC1[0]) = (0
                                 | 0x40u          // 转换完成中断,0为禁止，1为使能
                                   //| ADC_SC1_DIFF_MASK        // 差分模式使能,0为单端，1为差分
                                 | (((uint32_t)(((uint32_t)(ch))<<0))&0x1Fu)
                                     );
...
void adc_stop(ADCn_e adcn)
{
  ((ADCN[adcn])->SC1[0]) = (0
                                | 0x40u                       // 转换完成中断,0为禁止，1为使能
                                  //| ADC_SC1_DIFF_MASK                     // 差分模式使能,0为单端，1为差分
                                | (((uint32_t)(((uint32_t)(Module0_Dis))<<0))&0x1Fu)               //输入通道选择,此处选择禁止通道
                                    );
}
...
uint_16 adc_deal(int channel, uint8_t accurary, enum HardwareAverage N)
{
  ...
  {
  case 8:
    ADCCfg = (((uint32_t)(((uint32_t)(0))<<2))&0xCu);
    break;
  case 12:
    ADCCfg = (((uint32_t)(((uint32_t)(1))<<2))&0xCu);
    break;
  case 10:
    ADCCfg = (((uint32_t)(((uint32_t)(2))<<2))&0xCu);
    break;
  case 16:
    ADCCfg = (((uint32_t)(((uint32_t)(3))<<2))&0xCu);
    break;
  default:
    ADCCfg = (((uint32_t)(((uint32_t)(0))<<2))&0xCu);		
  }
  ((((ADC_MemMapPtr)0x4003B000u))->SC3) = 0 | 0x4u | (((uint32_t)(((uint32_t)((uint8_t)N))<<0))&0x3u);   //SC3寄存器硬件触发
  
  ADCCfg |=  ((((uint32_t)(((uint32_t)(2))<<5))&0x60u) | (((uint32_t)(((uint32_t)(1))<<0))&0x3u));
  
  ((((ADC_MemMapPtr)0x4003B000u))->CFG1) = ADCCfg;
  
  ((((ADC_MemMapPtr)0x4003B000u))->CFG2) = 0 | 0x4u;
  
  ((((ADC_MemMapPtr)0x4003B000u))->SC1[0]) = (((uint32_t)(((uint32_t)(channel))<<0))&0x1Fu);
  
  while((((((ADC_MemMapPtr)0x4003B000u))->SC1[0]) & 0x80u) != 0x80u)
  {
  }
  
  ADCResult = (uint_16)((((ADC_MemMapPtr)0x4003B000u))->R[0]);
  
  ((((ADC_MemMapPtr)0x4003B000u))->SC1[0]) &= ~0x80u;
